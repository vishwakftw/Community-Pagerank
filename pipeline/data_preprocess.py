...
p.add_argument('--stem', action='store_true', help='Stem instead of lemmatization')
...
for community in os.listdir(p.dir_root):
    ...
    if os.path.isdir(community_path):
        new_dir = os.path.join(p.clean_data, community)
        ...
        for person in os.listdir(community_path):
            ...
            if os.path.isfile(file_path):
                ...
                if do_stem:
                    processed = [porter.stem(word) for word in words]  # word stemming
                else:
                    processed = [wordnet_lemmatizer.lemmatize(word) for word in words] # word lemmating
                cleaned = [w for w in processed if w is not '']  # remove blank words
